...
자율주행 학습 데이터 관리 시스템을 구축하는 프로젝트입니다. 대규모 주행 데이터의 수집·전처리·검증·서빙 파이프라인을 목표로 합니다.
...
2) 객체 탐지 및 어노테이션 관리  
   - YOLO(ultralytics)로 차량/보행자/신호등 탐지  
   - 탐지 결과(바운딩 박스, 클래스, 신뢰도) PostgreSQL 저장  
   - 기존 어노테이션과 비교해 품질 검증  
   - 저품질(블러, 가림 등) 자동 필터링  
...
- Python, FastAPI
- PostgreSQL (메타데이터)
- MinIO 또는 로컬 파일시스템 (이미지 저장)
- OpenCV, Pillow (이미지 처리)
- YOLOv8 / ultralytics (객체 탐지)
